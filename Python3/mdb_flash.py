...
#    --mdb-path="/opt/microchip/mplabx/v5.45/mplab_platform/bin/mdb.sh" \
#    --mcu="PIC18F87K90" \
#    --hwtool="ICD3" \
...
def get_hwtool_index_by_serial(hwtool, serial, mdb_output): # {{{
  ...
  if (serial == None and len(matches) == 1):
    print("no serial was given, and only one hwtool is attached: just use it")
    ret = matches[0]['index']
  else:
    for item in matches:
      print("item:")
